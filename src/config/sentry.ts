    ...
    beforeSend(event, _hint) {
      ...
      if (event.contexts) {
        Object.keys(event.contexts).forEach(key => {
          event.contexts![key] = filterSensitiveData(event.contexts![key]) as Record<
            string,
            unknown
          >;
        });
...
function filterSensitiveData(data: unknown): unknown {
  ...
  Object.keys(filtered).forEach(key => {
    ...
    if (sensitiveKeys.some(sensitive => lowerKey.includes(sensitive))) {
      ...
      typeof (filtered as Record<string, unknown>)[key] === 'object' &&
      (filtered as Record<string, unknown>)[key] !== null
    ) {
      (filtered as Record<string, unknown>)[key] = filterSensitiveData(
...
export const sentryRequestHandler =
  (Sentry as any).Handlers?.requestHandler() || ((req: any, res: any, next: any) => next());

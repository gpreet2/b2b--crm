    ...
    return (
      ...
      >
        ...
        <div className="space-y-1 sm:space-y-2">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-1 min-w-0 flex-1">
              <span className="hidden sm:inline flex-shrink-0">{getTypeIcon(event.extendedProps.type)}</span>
        ...
        <div id={`workout-${event.id}-details`} className="sr-only">
          Workout type: {event.extendedProps.type}. 
          Exercises: {event.extendedProps.exercises.join(', ') || 'No exercises listed'}. 
        ...
        {isCompleted && (
          <div className="absolute bottom-0 left-0 right-0 h-1 bg-black/20 rounded-b-lg overflow-hidden">
            <div className="h-full bg-green-400 transition-all duration-500" style={{ width: '100%' }} />
          </div>
        )}
        
        {/* Hover effect overlay */}
        <div className="absolute inset-0 bg-white/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-lg pointer-events-none"></div>
      </div>
    )
  }
  ...
  const renderWeekView = () => {
    ...
    return (
      ...
      >
        ...
        <div className="grid grid-cols-7 bg-gradient-to-r from-surface-light to-accent border-b border-border flex-shrink-0" role="row">
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1 sm:space-y-2">
                  ...
                  <div className={`text-lg sm:text-xl md:text-2xl font-bold transition-colors duration-200 ${
                ...
                {/* Today indicator */}
                {isToday && (
                  <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-6 sm:w-8 h-1 bg-primary rounded-full"></div>
                )}
              </div>
            )
          })}
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col h-full p-2 sm:p-3 space-y-1 sm:space-y-2">
                  ...
                  <div className="flex-1 space-y-1 pt-1 sm:pt-2 overflow-y-auto" role="list" aria-label={`Workouts for ${date.toLocaleDateString()}`}>
                  ...
                  {dayEvents.length > 0 && (
                    <div id={`day-${index}-events`} className="sr-only">
                      {dayEvents.map((event, eventIndex) => (
                        <span key={event.id}>
                          {eventIndex > 0 && ', '}
                          {event.title} at {event.start.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      ))}
                    </div>
                  )}
                  ...
                  {dayEvents.length === 0 && (
                    <div className="flex-1 flex items-center justify-center">
                      <div className="opacity-0 group-hover:opacity-40 group-focus:opacity-40 transition-all duration-300 text-center transform translate-y-2 group-hover:translate-y-0 group-focus:translate-y-0">
                        <div className="w-8 sm:w-10 h-8 sm:h-10 mx-auto mb-1 sm:mb-2 rounded-full bg-accent/30 flex items-center justify-center" aria-hidden="true">
                          <PlusIcon className="h-4 sm:h-5 w-4 sm:w-5 text-secondary-text" />
                        </div>
                        <p className="text-xs text-secondary-text font-medium hidden sm:block">Add workout</p>
                      </div>
                    </div>
                  )}
  ...
  const renderMonthView = () => {
    ...
    return (
      ...
      >
        ...
        <div className="grid grid-cols-7 bg-gradient-to-r from-surface-light to-accent border-b border-border" role="row">
          {['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day) => (
            <div key={day} className="p-2 sm:p-3 md:p-4 text-center border-r border-border last:border-r-0" role="columnheader" aria-label={day}>
          ...
          {monthDates.map((date, index) => {
            ...
            return (
              ...
              >
                ...
                <div className="space-y-1 flex-1" role="list" aria-label={`Workouts for ${date.toLocaleDateString()}`}>
                  {visibleEvents.map((event) => (
                    <div
                      key={event.id}
                    ...
                    >
                      <div className="flex items-center space-x-1">
                        <span className="hidden sm:inline" aria-hidden="true">{getTypeIcon(event.extendedProps.type)}</span>
                ...
                {dayEvents.length === 0 && isCurrentMonth && (
                  <div className="flex items-center justify-center h-8 sm:h-12 md:h-16 opacity-0 group-hover:opacity-30 group-focus:opacity-30 transition-all duration-300">
                    <div className="text-center transform translate-y-2 group-hover:translate-y-0 group-focus:translate-y-0">
                      <div className="w-6 sm:w-8 h-6 sm:h-8 mx-auto mb-1 rounded-full bg-accent/30 flex items-center justify-center" aria-hidden="true">
    ...
    return (
      ...
      >
        ...
        <div id="list-view-description" className="sr-only">
          Table showing workout details including name, type, status, duration, time, and available actions. Use Tab to navigate between elements.
        </div>
        
        {/* Table Header */}
        <div className="grid grid-cols-6 gap-4 px-6 py-4 bg-accent border-b border-border" role="row">
          <div className="text-sm font-light text-secondary-text uppercase tracking-wider" role="columnheader">WORKOUT</div>
          <div className="text-sm font-light text-secondary-text uppercase tracking-wider" role="columnheader">TYPE</div>
          <div className="text-sm font-light text-secondary-text uppercase tracking-wider" role="columnheader">STATUS</div>
          <div className="text-sm font-light text-secondary-text uppercase tracking-wider" role="columnheader">DURATION</div>
          <div className="text-sm font-light text-secondary-text uppercase tracking-wider" role="columnheader">TIME</div>
          <div className="text-sm font-light text-secondary-text uppercase tracking-wider" role="columnheader">ACTIONS</div>
        </div>
          ...
          {visibleEvents.length === 0 ? (
            <div className="px-6 py-12 text-center" role="row">
              <div role="cell" className="col-span-6">
                <ClockIcon className="h-12 w-12 text-secondary-text mx-auto mb-4" aria-hidden="true" />

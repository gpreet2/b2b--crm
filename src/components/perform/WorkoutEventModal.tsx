...
export default function WorkoutEventModal({ isOpen, onClose, event }: WorkoutEventModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className='w-full max-w-lg max-h-[90vh] overflow-y-auto p-0'>
          ...
          <div className='flex items-start justify-between'>
            <div className='flex-1'>
              <div className='flex items-center space-x-3 mb-3'>
                ...
                <div className='flex-1'>
                  <DialogTitle className='text-xl font-semibold text-primary-text mb-1'>
                    {event.title}
                  </DialogTitle>
        ...
        <div className='p-6 space-y-6'>
          ...
          <div className='grid grid-cols-2 gap-4'>
            <div className='p-4 bg-gradient-to-br from-surface-light/30 to-surface-light/10 rounded-xl backdrop-blur-sm'>
              ...
              <p className='text-lg font-semibold text-primary-text'>
                {event.extendedProps.duration} minutes
              </p>
            </div>
            ...
            <div className='p-4 bg-gradient-to-br from-surface-light/30 to-surface-light/10 rounded-xl backdrop-blur-sm'>
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-medium ${getIntensityColor(event.extendedProps.intensity)}`}
              >
                {event.extendedProps.intensity.charAt(0).toUpperCase() +
                  event.extendedProps.intensity.slice(1)}
              </span>
            </div>
          </div>
          ...
          {/* Notes */}
          {event.extendedProps.notes ? <div className='bg-gradient-to-br from-info/10 to-info/5 rounded-xl p-4 backdrop-blur-sm'>
              <h3 className='text-lg font-semibold text-primary-text mb-3 flex items-center space-x-2'>
                <div className='p-1.5 bg-info/20 rounded-lg'>
        ...
        <div className='flex items-center justify-end space-x-3 p-6 bg-gradient-to-r from-surface-light/30 to-surface-light/20'>
          ...
          <Button className='px-4 bg-gradient-to-r from-primary to-primary-dark hover:from-primary-dark hover:to-primary text-white flex items-center space-x-2'>
            {event.extendedProps.completed ? (
              <>

...
export default function ClassCalendar({
  ...
  const classesByDate = useMemo(() => {
    return visibleClasses.reduce((acc, classItem) => {
  ...
  const renderClassEvent = (classItem: Class) => {
    ...
    return (
      ...
      >
        ...
        {coach && (
          <div className="text-xs opacity-75 truncate">{coach.name}</div>
        )}
  ...
  const renderWeekView = () => {
    ...
    return (
      <div className="grid grid-cols-8 gap-px bg-gray-200 rounded-lg overflow-hidden">
        {/* Time column header */}
        <div className="bg-gray-50 p-2 text-xs font-medium text-gray-600">
          Time
        </div>

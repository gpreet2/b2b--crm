...
const Header = React.forwardRef<HTMLDivElement, HeaderProps>(
  ({ className, breadcrumbs, onMenuToggle, user, notifications = 0, ...props }, ref) => {
    return (
      <header
        ref={ref}
        className={cn(
      ...
      >
              ...
              {user ? (
                <div className='flex items-center space-x-3'>
                  <div className='text-right hidden sm:block'>
                    ...
                    <p className='text-xs text-secondary-text font-normal'>{user.email}</p>
                  </div>
                ...
                <Button variant='primary' size='sm' className='hidden sm:inline-flex hover-lift'>
                  Sign In
                </Button>
              )}
            </div>
        ...
        {/* Mobile Breadcrumbs */}
        {breadcrumbs && breadcrumbs.length > 0 ? <div className='mt-3 sm:hidden border-t border-border-light pt-3'>
            <Breadcrumb items={breadcrumbs} />
          </div> : null}
      </header>
    );

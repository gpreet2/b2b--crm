...
const Sidebar = React.forwardRef<HTMLDivElement, SidebarProps>(
  ({ className, onClose, ...props }, ref) => {
    ...
    const locations = ['Bakersfield, CA', 'Los Angeles, CA', 'San Francisco, CA', 'Sacramento, CA'];
                  ...
                  >
                    {locations.map((location, index) => (
                      <button
                        ...
                        style={{
                          backgroundColor: selectedLocation === location ? '#fef2f2' : '#ffffff',
                        }}
                        onMouseEnter={e => {
                        ...
                        onMouseLeave={e => {
                          e.currentTarget.style.backgroundColor =
                            selectedLocation === location ? '#fef2f2' : '#ffffff';
                        }}
                      >
                        <div
                          className={`w-6 h-6 rounded-full flex items-center justify-center ${
                            selectedLocation === location ? 'bg-primary/20' : 'bg-gray-100'
                          }`}
                        >
                          <MapPinIcon
                            className={`h-3 w-3 ${
                              selectedLocation === location ? 'text-primary' : 'text-gray-400'
                            }`}
                          />
                        </div>
                        <span
                  ...
                  {item.expandable ? (
                    <div>
                      <div className='flex items-center'>
                        ...
                        >
                          {/* Active indicator */}
                          {isActive ? <div className='absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-primary to-primary-light rounded-r-full' /> : null}
                          ...
                          {item.badge ? <span className='px-1.5 py-0.5 text-xs bg-primary text-white rounded-full font-bold shadow-sm'>
                              {item.badge}
                            </span> : null}
                        </button>
                        ...
                        <button
                          ...
                          aria-label={`${isExpanded ? 'Collapse' : 'Expand'} ${item.title} menu`}
                          title={`${isExpanded ? 'Collapse' : 'Expand'} ${item.title} submenu`}
                        >
                          <ChevronDownIcon
                            className={cn(
                      ...
                      {/* Sub Items */}
                      {isExpanded && item.subItems ? <div className='ml-6 mt-2 space-y-1.5 pl-4 border-l-2 border-border-light'>
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className='flex items-center justify-between'>
                                  ...
                                  {subItem.badge ? <span className='px-1.5 py-0.5 text-xs bg-primary text-white rounded-full font-bold shadow-sm'>
                                      {subItem.badge}
                                    </span> : null}
                                </div>
                              </Link>

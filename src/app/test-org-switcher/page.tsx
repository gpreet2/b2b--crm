  ...
  return (
    <div className='min-h-screen bg-gradient-to-br from-surface via-surface-light to-accent p-8'>
      <div className='max-w-4xl mx-auto space-y-6'>
        <Card className='p-8'>
          <h1 className='text-2xl font-bold text-primary-text mb-6'>Test Organization Switcher</h1>

          {error ? <div className='mb-4 p-4 bg-danger/10 border border-danger/20 rounded-lg'>
          ...
          <div className='space-y-4'>
            ...
            <div className='space-y-2'>
              ...
              {loading ? <p className='text-secondary-text'>Loading organizations...</p> : null}
              ...
              {organizations.map(org => (
                <div
                  ...
                  className={`p-4 rounded-lg border ${
                    org.isCurrent ? 'border-primary bg-primary/5' : 'border-border bg-surface'
                  }`}
                >
                  <div className='flex items-center justify-between'>
                    ...
                    {org.isCurrent ? <span className='text-sm text-primary font-medium'>✓ Current</span> : null}
                  </div>
                </div>
              ))}
            </div>

            {switchResult ? <div className='mt-6 p-4 bg-success/10 border border-success/20 rounded-lg'>
                <h3 className='font-medium text-success mb-2'>Switch Successful!</h3>
                <pre className='text-xs overflow-auto'>{JSON.stringify(switchResult, null, 2)}</pre>
              </div> : null}
          </div>
        </Card>

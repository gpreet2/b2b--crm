...
class MobileStorage {
  ...
  async save(key: string, value: any) {
    ...
    console.log(`📱 Saved to mobile storage: ${key}`);
  }
...
class MobileAuthManager {
  private storage = new MobileStorage();
  ...
  async checkAuthStatus() {
    ...
    try {
      ...
      if (data.authenticated) {
        ...
        await this.storage.save('user', data.user);
        await this.storage.save('organizationId', data.organizationId);
        await this.storage.save('sessionExpiry', data.sessionExpiry);
        
        // Check if refresh needed
        if (data.needsRefresh) {
          console.log('   ⚠️  Token expires soon, need to refresh');
  ...
  async refreshToken(organizationId?: string) {
    ...
    try {
      ...
      if (data.success) {
        ...
        await this.storage.save('organizationId', data.organizationId);
        await this.storage.save('sessionExpiry', data.sessionExpiry);
        await this.storage.save('nextRefreshTime', 
          new Date(Date.now() + (data.refreshIn || 0)).toISOString()
        );
        
        console.log('   ✅ Token refreshed successfully');
        console.log(`   📅 Next refresh scheduled for: ${await this.storage.get('nextRefreshTime')}`);
      }
...
async function runMobileAppSimulation() {
  ...
  console.log(`   Result: User is ${initialStatus.authenticated ? 'authenticated' : 'not authenticated'}`);
  ...
  const refreshWithoutAuth = await mobileAuth.refreshToken();

 ...
 */
export async function createAuditLog(entry: Partial<AuditLogEntry> & { action: string; entity_type: string }): Promise<void> {
  try {
...
export const auditAuth = {
  ...
  failedLogin: (req: Request, email?: string, attemptCount = 1) =>
    auditAction(req, AUDITABLE_ACTIONS.AUTH_FAILED_LOGIN, RESOURCE_TYPES.USER, undefined, {
      email,
...
export const auditSecurity = {
  permissionDenied: (req: Request, requiredPermission: string, entityType: string, entityId?: string) =>
    auditAction(req, AUDITABLE_ACTIONS.SECURITY_PERMISSION_DENIED, entityType, entityId, {
      required_permission: requiredPermission,
    }, 'failure'),
    
  rateLimitExceeded: (req: Request, limitType: string, limit: number) =>
    auditAction(req, AUDITABLE_ACTIONS.SECURITY_RATE_LIMIT_EXCEEDED, RESOURCE_TYPES.SYSTEM, undefined, {
  ...
  suspiciousActivity: (req: Request, activityType: string, details: Record<string, any>) =>
    auditAction(req, AUDITABLE_ACTIONS.SECURITY_SUSPICIOUS_ACTIVITY, RESOURCE_TYPES.SYSTEM, undefined, {
...
export const auditCRUD = {
  read: (entityType: string) => auditMiddleware(
    AUDITABLE_ACTIONS.DATA_READ,
    entityType,
    {
  ...
  create: (entityType: string) => auditMiddleware(
    AUDITABLE_ACTIONS.DATA_CREATE,
    entityType,
    {
  ...
  update: (entityType: string) => auditMiddleware(
    AUDITABLE_ACTIONS.DATA_UPDATE,
    entityType,
    {
      extractEntityId: (req) => req.params.id,
  ...
  delete: (entityType: string) => auditMiddleware(
    AUDITABLE_ACTIONS.DATA_DELETE,
    entityType,
    {
      extractEntityId: (req) => req.params.id,

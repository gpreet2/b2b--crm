 ...
 */
router.get('/requests/:id', requirePermission('privacy:read'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const organizationId = req.headers['x-organization-id'] as string;
 ...
 */
router.post('/requests/:id/fulfill/access', requirePermission('privacy:fulfill'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    if (format === 'csv') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="data-access-${request.user_id}.zip"`);
      archive.pipe(res);
 ...
 */
router.post('/requests/:id/fulfill/erasure', requirePermission('privacy:fulfill'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const { data: auditLogs } = await supabase
      ...
      .in('action', ['auth.login', 'auth.logout', 'billing.payment', 'security.suspicious_activity']);
 ...
 */
router.post('/requests/:id/fulfill/rectification', requirePermission('privacy:fulfill'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/consent/:userId', requirePermission('privacy:read'), async (req: Request, res: Response) => {
  try {
 ...
 */
router.post('/consent/:userId', requirePermission('privacy:manage'), async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
